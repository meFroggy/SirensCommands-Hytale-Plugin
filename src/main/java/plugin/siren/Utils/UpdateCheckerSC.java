...
public class UpdateCheckerSC {
    ...
    public static void sendUpdateMessage(Type type){
        if(Type.StartUp.getValue() == type.getValue()) {
            ...
            if(!MermaidCommands.getVersion().equalsIgnoreCase(recentVersion)) {
                MermaidCommands.LOGGER.atInfo().log("= =- -=- -=- -=- -=- -=- -=- -=- -=- -=- -= =");
                MermaidCommands.LOGGER.atInfo().log("The Chinese Festivals Mod version is outdated, Chinese Festivals has released v" + recentVersion + ".");
            }
    ...
    public static void sendUpdateMessage(@Nullable Player player, boolean sendToPlayer){
        ...
        if(!MermaidCommands.getVersion().equalsIgnoreCase(recentVersion)){
            ...
            MermaidCommands.LOGGER.atInfo().log(versionMessage.getAnsiMessage());
